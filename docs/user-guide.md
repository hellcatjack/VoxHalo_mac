...
The translation region gives the complete newest sentence a slightly brighter fill and makes confirmed history slightly quieter. With the default white setting, history is light gray and the newest sentence stays white; colored choices use the same relative difference. When the next sentence arrives, the previous one changes to the history shade exactly once and the new active sentence becomes bright. There is no blinking or color animation, and recognition subtitles keep their selected color unchanged.
