...
VoxHalo stabilizes translated text locally. Once a newer source sentence appears, earlier rendered translations no longer change even if the backend later revises its canonical result. A punctuation-complete prefix is frozen sooner. Only the unfinished current sentence may receive one structural correction, and only after the backend explicitly revises that sentence's source. When the next sentence arrives or the session finishes, the latest translation for that active sentence is committed once and then frozen. Partial aggregate fallback text can grow but cannot rewrite what is already visible. Append-only growth remains visible, and ordinary shorter intermediate rollbacks are ignored.
