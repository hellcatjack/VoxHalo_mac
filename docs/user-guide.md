...
VoxHalo automatically adds a font-scaled high-contrast outline and a soft dark halo. Light subtitle colors receive a dark outline; a dark custom color receives a light outline. The video remains visible because this protection is local to the glyphs rather than an opaque band, and the text stays distinguishable when its fill color matches a bright, dark, or colored part of the video.

The translation region gives the complete newest sentence a clearly brighter fill and makes confirmed history quieter. The newest sentence is blended 10% toward white and history 24% toward black. With the default white setting, history is `#C2C2C2` and the newest sentence stays `#FFFFFF`; colored choices preserve the same strong visual hierarchy. When the next sentence arrives, the previous one changes to the history shade exactly once and the new active sentence becomes bright. There is no blinking or color animation, and recognition subtitles keep their selected color unchanged.
...
The target keeps a multi-minute history as complete sentences instead of continuously deleting characters from the beginning. A backend final-redecode or reconciliation keeps the current paragraph visible while rebuilding, then switches to the final result in one update. These behaviors are entirely client-side and require no backend change.
