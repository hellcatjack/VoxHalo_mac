...
第一阶段将当前中英双向实现中的可复用代码从 CLI 服务入口分离。只开放中文普通话与英语；其他六种语言尚未接通。模型、提示词、语速、句子固化和音频提交算法沿用现有配置。

## 模块分工

| 模块 | 职责 |
|---|---|
| `voxbridge/languages.py` | 不可变语言配置与语言对；严格拒绝未开放语言或相同源/目标语言 |
| `interpretation/contracts.py` | 不可变翻译请求、会话独享的有界队列和最新修订索引 |
| `interpretation/translation_queue.py` | 翻译任务并行限制、等待和取消清理 |
| `interpretation/transcript.py` | 原文切分策略接口及现有中英策略 |
...
第一阶段保留原生 Swift 的两个方向、旧协议默认值、复杂 ASR 修订状态机以及既有语音发布器。尚未将整个 WebSocket 会话改成完全独立的通用会话类，也不宣称已支持 56 个方向。
