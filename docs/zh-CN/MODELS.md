...
Kokoro 是小型神经 TTS 模型，不承担文本翻译。两个约 8,200 万参数型号均使用 `kokoro-onnx 0.5.0`、ONNX Runtime 1.30.0、两个 CPU 线程和 Auto 追赶，让 GPU 优先处理识别和翻译。英文保留基础速度 1.05；中文自动合成采用 1.10–1.20 的绝对速度范围，档位为 1.10、1.14、1.18、1.20。显式固定语速模式仍按配置执行。
