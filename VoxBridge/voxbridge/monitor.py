...
</style></head><body><header><img src="/listen/qr.svg" alt="局域网朗读二维码"><h1>同声传译监控</h1><p class="muted">识别与朗读由本机 App 运行。关闭此页不影响业务。</p><div class="status"><span id="session" class="pill">正在连接…</span><span id="tts" class="pill">读取朗读状态…</span><span id="listeners" class="pill"></span></div><p id="error" class="error"></p><div class="tools"><label class="muted"><input id="follow" type="checkbox" checked> 跟随最新字幕</label><a href="/listen" target="_blank" rel="noopener">听众朗读页</a></div></header><main><div class="labels"><span id="sourceLabel">中文原文</span><span id="targetLabel">英文译文</span></div><div id="rows"></div></main><script>
